...
kV, kQ = args.p, 1 if args.p is 1 else 1
V = VectorFunctionSpace(mesh, 'CG', kV)
Q = FunctionSpace(mesh, 'CG', kQ)
...
if kV == kQ: # stabilized equal order
  h = CellSize(mesh)
  delta = Constant(1./12)
  stokes -= h**2*dot(delta*grad(p), grad(q))*dx
...
# mass correction
if args.correct:
  div_uh = project(div(uh), FunctionSpace(mesh, 'DG', kV-1));
  if kV == kQ: # stabilized equal order
    uh = project(uh - h**2*delta*grad(ph), VectorFunctionSpace(mesh, 'DG', 1));

  masscorrection.compute_correction(mesh, uh, div_uh, Q, kQ)
